...
<head>
    ...
    <title>{{ __('app.checkout.success_meta_title', ['app' => config('app.name', 'Harmoniva')]) }}</title>
...
<div class="flex items-center justify-center px-4 py-16"
    ...
    <div class="max-w-lg w-full bg-white rounded-2xl border border-gray-100 shadow-lg p-8 sm:p-10 text-center">
        ...
        <h1 class="text-2xl sm:text-3xl font-extrabold text-gray-900 mb-3">{{ __('app.checkout.success_title') }}</h1>
        @php $successPlan = $subscription->plan->name ?? 'Premium'; @endphp
        <p class="text-gray-500 mb-2">
            @if($subscription->ends_at)
                {!! __('app.checkout.success_active_until', ['plan' => e($successPlan), 'date' => $subscription->ends_at->translatedFormat('j M Y')]) !!}
            @else
                {!! __('app.checkout.success_active', ['plan' => e($successPlan)]) !!}
            @endif
        </p>
        <p class="text-sm text-gray-400 mb-8">{{ __('app.checkout.success_invoice', ['number' => optional($subscription->invoices()->latest()->first())->invoice_number]) }}</p>
        ...
        <div class="flex flex-col sm:flex-row gap-3 justify-center">
            <a href="{{ route('dashboard') }}"
               ...
               style="background:linear-gradient(135deg,#9333ea,#7c3aed);">
                <i data-lucide="layout-dashboard" style="width:16px;height:16px;"></i> {{ __('app.checkout.success_dashboard') }}
            </a>
            <a href="{{ route('billing.index') }}"
               class="inline-flex items-center justify-center gap-2 px-6 py-3 text-sm font-semibold text-gray-700 bg-white border border-gray-200 rounded-xl hover:border-purple-300 transition-all">
                {{ __('app.checkout.success_billing') }}
            </a>
        </div>
    </div>
</div>
