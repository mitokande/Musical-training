...
<head>
    ...
    <title>{{ __('app.teacher.dashboard_title') }} - {{ config('app.name', 'Harmoniva') }}</title>
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="bg-gradient-to-r from-purple-600 via-purple-500 to-orange-400 rounded-2xl p-8 text-white mb-8">
        ...
        <p class="text-purple-100">{{ __('app.teacher.dashboard_subtitle') }}</p>
    </div>

    {{-- Quick Actions --}}
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <a href="{{ route('teacher.profile.edit') }}" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition group">
            <div class="w-12 h-12 rounded-xl bg-purple-100 flex items-center justify-center mb-4 group-hover:bg-purple-200 transition">
                <i data-lucide="user-circle" class="w-6 h-6 text-purple-600"></i>
            </div>
            <h3 class="font-semibold text-gray-900 mb-1">{{ __('app.teacher.my_profile') }}</h3>
            <p class="text-sm text-gray-500">{{ __('app.teacher.my_profile_desc') }}</p>
        </a>
        ...
        <a href="{{ route('articles.index') }}" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition group">
            ...
            <h3 class="font-semibold text-gray-900 mb-1">{{ __('app.teacher.my_content') }}</h3>
            <p class="text-sm text-gray-500">{{ __('app.teacher.my_content_desc') }}</p>
        </a>
        ...
        <a href="{{ route('profile.edit') }}" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition group">
            ...
            <h3 class="font-semibold text-gray-900 mb-1">{{ __('app.teacher.account_settings') }}</h3>
            <p class="text-sm text-gray-500">{{ __('app.teacher.account_settings_desc') }}</p>
        </a>
    </div>
    ...
    <div class="bg-blue-50 border border-blue-200 rounded-2xl p-6">
        <div class="flex items-start gap-4">
            ...
            <div>
                <h3 class="font-semibold text-blue-900 mb-1">{{ __('app.teacher.coming_soon') }}</h3>
                <p class="text-sm text-blue-700">{{ __('app.teacher.coming_soon_desc') }}</p>
            </div>
        </div>
    </div>
</div>
