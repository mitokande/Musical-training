...
@section('title', crm_trans('appointments.title'))
...
<div class="mb-6">
    <h1 class="text-2xl font-bold text-gray-900">{{ crm_trans($ap.'.title') }}</h1>
    <p class="text-gray-500 text-sm mt-1">{{ crm_trans($ap.'.subtitle') }}</p>
</div>
...
<div class="grid lg:grid-cols-3 gap-6">
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="card overflow-hidden">
            <div class="px-5 py-3.5 border-b border-gray-100">
                <h2 class="font-bold text-gray-900">{{ crm_trans($ap.'.pending_requests') }} ({{ $pending->count() }})</h2>
            </div>
            @if($pending->isEmpty())
                <div class="p-6 text-sm text-gray-400">{{ crm_trans($ap.'.no_pending') }}</div>
            @else
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="font-semibold text-gray-900">
                                        ...
                                        <span class="ml-2 px-2 py-0.5 rounded-full text-[11px] font-semibold {{ $statusColors[$appointment->status] ?? '' }}">
                                            {{ crm_trans($ap.'.status_'.$appointment->status) }}
                                        </span>
                                    </p>
                                    ...
                                    @if($appointment->status === 'reschedule_requested' && $appointment->requested_starts_at)
                                        <p class="text-sm text-blue-700 mt-0.5">
                                            {{ crm_trans($ap.'.reschedule_to') }}: {{ $appointment->requested_starts_at->timezone($tz)->format('D, M j, Y H:i') }}
                                        </p>
                                    @endif
                                    @if($appointment->topic)
                                        <p class="text-xs text-gray-400 mt-1">{{ crm_trans($ap.'.topic') }}: {{ $appointment->topic }}</p>
                                    @endif
                                </div>
                                <div class="flex flex-col gap-2 shrink-0 w-full sm:w-56">
                                    <form method="POST" action="{{ crm_route('appointments.confirm', $appointment) }}" class="flex flex-col gap-1.5">
                                        @csrf
                                        <input type="url" name="meeting_url" placeholder="{{ crm_trans($ap.'.meeting_url') }}" class="rounded-lg border-gray-300 text-xs">
                                        <button class="w-full py-2 bg-green-600 hover:bg-green-700 text-white text-xs font-semibold rounded-lg transition">{{ crm_trans($ap.'.confirm') }}</button>
                                    </form>
                                    <form method="POST" action="{{ crm_route('appointments.reject', $appointment) }}">
                                        @csrf
                                        <button class="w-full py-2 bg-gray-100 hover:bg-gray-200 text-gray-600 text-xs font-semibold rounded-lg transition">{{ crm_trans($ap.'.reject') }}</button>
                                    </form>
                                </div>
                            </div>
                        </li>
        ...
        <div class="card overflow-hidden">
            <div class="px-5 py-3.5 border-b border-gray-100">
                <h2 class="font-bold text-gray-900">{{ crm_trans($ap.'.upcoming') }}</h2>
            </div>
            @if($upcoming->isEmpty())
                <div class="p-6 text-sm text-gray-400">{{ crm_trans($ap.'.no_upcoming') }}</div>
            @else
                <ul class="divide-y divide-gray-100">
                    @foreach($upcoming as $appointment)
                        <li class="px-5 py-4" x-data="{ open: false }">
                            ...
                            <div x-show="open" x-cloak class="mt-3 pt-3 border-t border-gray-100 grid sm:grid-cols-2 gap-2">
                                <form method="POST" action="{{ crm_route('appointments.reschedule', $appointment) }}" class="flex gap-1.5">
                                    @csrf
                                ...
                                <div class="flex gap-2">
                                    <form method="POST" action="{{ crm_route('appointments.complete', $appointment) }}" class="flex-1">
                                        @csrf
                                        <button class="w-full py-1.5 bg-primary-50 text-primary-700 text-xs font-semibold rounded-lg hover:bg-primary-100">{{ crm_trans($ap.'.complete') }}</button>
                                    </form>
                                    <form method="POST" action="{{ crm_route('appointments.no-show', $appointment) }}" class="flex-1">
                                        @csrf
                                        <button class="w-full py-1.5 bg-gray-100 text-gray-600 text-xs font-semibold rounded-lg hover:bg-gray-200">{{ crm_trans($ap.'.no_show') }}</button>
                                    </form>
                                    <form method="POST" action="{{ crm_route('appointments.cancel', $appointment) }}" class="flex-1">
                                        @csrf
                                        <button class="w-full py-1.5 bg-red-50 text-red-600 text-xs font-semibold rounded-lg hover:bg-red-100">{{ crm_trans($ap.'.cancel') }}</button>
                                    </form>
                                </div>
                                <form method="POST" action="{{ crm_route('appointments.note', $appointment) }}" class="sm:col-span-2 flex gap-1.5">
                                    @csrf @method('PUT')
                                    <input type="text" name="teacher_note" value="{{ $appointment->teacher_note }}" placeholder="{{ crm_trans($ap.'.note') }}" class="flex-1 rounded-lg border-gray-300 text-xs">
        ...
        <div class="card overflow-hidden">
            <div class="px-5 py-3.5 border-b border-gray-100">
                <h2 class="font-bold text-gray-900">{{ crm_trans($ap.'.past') }}</h2>
            </div>
            <ul class="divide-y divide-gray-100">
                @foreach($past as $appointment)
                    <li class="px-5 py-3 flex items-center gap-3">
                        ...
                        @if($appointment->status === 'confirmed')
                            <form method="POST" action="{{ crm_route('appointments.complete', $appointment) }}">
                                @csrf
                                <button class="text-xs font-semibold text-primary-600 hover:text-primary-800">{{ crm_trans($ap.'.complete') }}</button>
                            </form>
                        @endif
                    </li>
                @endforeach
    ...
    <div class="space-y-6">
        {{-- Booking settings --}}
        <div class="card p-5">
            <h2 class="font-bold text-gray-900 mb-3">{{ crm_trans($ap.'.settings') }}</h2>
            <form method="POST" action="{{ crm_route('calendar.settings') }}" class="space-y-3">
                @csrf @method('PUT')
                <label class="flex items-center gap-2 text-sm font-semibold text-gray-700 cursor-pointer">
                    ...
                    {{ crm_trans($ap.'.booking_enabled') }}
                </label>
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 mb-1">{{ crm_trans($ap.'.lesson_duration') }}</label>
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 mb-1">{{ crm_trans($ap.'.buffer') }}</label>
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 mb-1">{{ crm_trans($ap.'.advance_days') }}</label>
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 mb-1">{{ crm_trans($ap.'.min_notice') }}</label>
                ...
                <div>
                    <label class="block text-xs font-semibold text-gray-500 mb-1">{{ crm_trans($ap.'.timezone') }}</label>
                ...
                <button class="w-full py-2 bg-primary-600 hover:bg-primary-700 text-white text-sm font-semibold rounded-lg transition">{{ crm_trans($ap.'.save') }}</button>
            </form>
        </div>

        {{-- Weekly availability --}}
        <div class="card p-5">
            <h2 class="font-bold text-gray-900 mb-3">{{ crm_trans($ap.'.weekly_availability') }}</h2>
            @if($availabilities->isEmpty())
                <p class="text-sm text-gray-400 mb-3">{{ crm_trans($ap.'.no_windows') }}</p>
            @else
                <ul class="space-y-1.5 mb-3">
                    @foreach($availabilities as $window)
                        <li class="flex items-center gap-2 text-sm text-gray-700">
                            ...
                            <form method="POST" action="{{ crm_route('calendar.availability.destroy', $window) }}">
                                @csrf @method('DELETE')
            ...
            @endif
            <form method="POST" action="{{ crm_route('calendar.availability.store') }}" class="flex gap-1.5">
                @csrf
        ...
        {{-- Exceptions --}}
        <div class="card p-5">
            <h2 class="font-bold text-gray-900 mb-3">{{ crm_trans($ap.'.exceptions') }}</h2>
                    ...
                    @foreach($exceptions as $exception)
                        <li class="flex items-center gap-2 text-sm text-gray-700">
                            <span class="flex-1">
                                ...
                                · {{ $exception->start_time ? substr($exception->start_time, 0, 5).'–'.substr($exception->end_time, 0, 5) : crm_trans($ap.'.whole_day') }}
                                <span class="text-[11px] font-semibold {{ $exception->is_blocked ? 'text-red-500' : 'text-green-600' }}">
                                    {{ $exception->is_blocked ? crm_trans($ap.'.blocked') : crm_trans($ap.'.extra') }}
                                </span>
                            </span>
                            <form method="POST" action="{{ crm_route('calendar.exceptions.destroy', $exception) }}">
                                @csrf @method('DELETE')
                                <button class="text-xs text-red-500 hover:text-red-700 font-semibold">×</button>
                            </form>
                        </li>
            ...
            @endif
            <form method="POST" action="{{ crm_route('calendar.exceptions.store') }}" class="space-y-1.5">
                @csrf
                <div class="flex gap-1.5">
                    ...
                    <select name="is_blocked" class="rounded-lg border-gray-300 text-xs">
                        <option value="1">{{ crm_trans($ap.'.blocked') }}</option>
                        <option value="0">{{ crm_trans($ap.'.extra') }}</option>
                    </select>
                </div>
                <div class="flex gap-1.5">
