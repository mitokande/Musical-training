...
@section('title', __('teacher.classes.title'))
...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6" x-data>
    <div>
        <h1 class="text-2xl font-bold text-gray-900">{{ __('teacher.classes.title') }}</h1>
        <p class="text-gray-500 text-sm mt-1">{{ __('teacher.classes.subtitle') }}</p>
    </div>
    <button @click="$dispatch('open-class-form')" class="inline-flex items-center gap-2 px-5 py-2.5 bg-primary-600 hover:bg-primary-700 text-white font-semibold rounded-lg transition">
        <i data-lucide="plus" class="w-4 h-4"></i> {{ __('teacher.classes.create') }}
    </button>
</div>
...
@if($classes->isEmpty())
    <div class="card p-10 text-center text-gray-500">
        ...
        <p class="text-sm">{{ __('teacher.classes.no_classes') }}</p>
    </div>
@else
    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        @foreach($classes as $class)
            <a href="{{ route('teacher.classes.show', $class) }}" class="card p-5 hover:shadow-md transition {{ $class->isArchived() ? 'opacity-60' : '' }}">
                <div class="flex items-start justify-between mb-2">
                    <div class="w-10 h-10 rounded-xl bg-primary-100 flex items-center justify-center">
                        <i data-lucide="school" class="w-5 h-5 text-primary-600"></i>
                    </div>
                    @if($class->isArchived())
                        <span class="text-[11px] font-semibold text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">{{ __('teacher.classes.archived') }}</span>
                    @endif
                </div>
                <h3 class="font-bold text-gray-900">{{ $class->name }}</h3>
                <p class="text-xs text-gray-500 mt-0.5">
                    {{ __('teacher.classes.students_count', ['count' => $class->students_count]) }}
                    @if($class->level) · {{ __('teacher.assignments.difficulty_'.$class->level) }} @endif
...
<div x-data="{ open: {{ $errors->any() ? 'true' : 'false' }} }" @open-class-form.window="open = true" x-show="open" x-cloak class="fixed inset-0 z-50 flex items-start justify-center p-4 pt-24">
    ...
    <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-bold text-gray-900">{{ __('teacher.classes.create') }}</h3>
            <button @click="open = false" class="p-1 text-gray-400 hover:text-gray-600"><i data-lucide="x" class="w-5 h-5"></i></button>
        </div>
        <form method="POST" action="{{ route('teacher.classes.store') }}" class="space-y-3">
            @csrf
            <div>
                <label class="block text-xs font-semibold text-gray-500 mb-1">{{ __('teacher.classes.name') }}</label>
                <input type="text" name="name" required maxlength="100" class="w-full rounded-lg border-gray-300 text-sm" value="{{ old('name') }}">
            </div>
            <div>
                <label class="block text-xs font-semibold text-gray-500 mb-1">{{ __('teacher.classes.description') }}</label>
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    <label class="block text-xs font-semibold text-gray-500 mb-1">{{ __('teacher.classes.level') }}</label>
                    <select name="level" class="w-full rounded-lg border-gray-300 text-sm">
                        <option value="">—</option>
                        @foreach(['beginner', 'intermediate', 'advanced'] as $level)
                            <option value="{{ $level }}">{{ __('teacher.assignments.difficulty_'.$level) }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-500 mb-1">{{ __('teacher.classes.instrument_focus') }}</label>
            ...
            <button class="w-full py-2.5 bg-primary-600 hover:bg-primary-700 text-white font-semibold rounded-lg transition">{{ __('teacher.classes.create') }}</button>
        </form>
    </div>
</div>
@endsection
