...
<a href="{{ route('teacher.classes.index') }}" class="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700 mb-4">
    <i data-lucide="arrow-left" class="w-4 h-4"></i> {{ __('teacher.classes.title') }}
</a>
...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div>
        <h1 class="text-2xl font-bold text-gray-900 flex items-center gap-2">
            {{ $class->name }}
            @if($class->isArchived())
                <span class="text-xs font-semibold text-gray-400 bg-gray-100 px-2 py-1 rounded-full">{{ __('teacher.classes.archived') }}</span>
            @endif
        </h1>
    ...
    <form method="POST" action="{{ route('teacher.classes.archive', $class) }}">
        ...
        <button class="px-4 py-2 text-sm font-semibold rounded-lg transition {{ $class->isArchived() ? 'bg-green-50 text-green-700 hover:bg-green-100' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }}">
            {{ $class->isArchived() ? __('teacher.classes.restore') : __('teacher.classes.archive') }}
        </button>
    </form>
</div>
...
<div class="grid grid-cols-3 gap-3 mb-6">
    <div class="card p-4">
        ...
        <p class="text-xs text-gray-500">{{ __('teacher.classes.summary_assignments') }}</p>
    </div>
    <div class="card p-4">
        <p class="text-xl font-bold text-gray-900">{{ $summary['completed'] }}</p>
        <p class="text-xs text-gray-500">{{ __('teacher.classes.summary_completed') }}</p>
    </div>
    <div class="card p-4">
        <p class="text-xl font-bold text-gray-900">{{ $summary['average_score'] !== null ? round($summary['average_score'], 1).'%' : '—' }}</p>
        <p class="text-xs text-gray-500">{{ __('teacher.classes.summary_avg') }}</p>
    </div>
</div>
...
<div class="grid lg:grid-cols-3 gap-6">
    <div class="lg:col-span-2">
        <div class="card overflow-hidden">
            <div class="px-5 py-3.5 border-b border-gray-100 flex items-center justify-between">
                <h2 class="font-bold text-gray-900">{{ __('teacher.nav.students') }} ({{ $students->count() }})</h2>
            </div>
            @if($students->isEmpty())
                <div class="p-8 text-center text-sm text-gray-400">{{ __('teacher.classes.no_students') }}</div>
            @else
                <ul class="divide-y divide-gray-100">
                    @foreach($students as $student)
                        <li class="flex items-center gap-3 px-5 py-3">
                            ...
                            @endif
                            <a href="{{ route('teacher.students.show', $student) }}" class="flex-1 text-sm font-semibold text-gray-800 hover:text-primary-700 truncate">
                                {{ $student->name }} {{ $student->surname }}
                            </a>
                            <form method="POST" action="{{ route('teacher.classes.students.remove', [$class, $student]) }}">
                                @csrf @method('DELETE')
    ...
    <div class="space-y-6">
        ...
        <div class="card p-5">
            <h2 class="font-bold text-gray-900 mb-3">{{ __('teacher.classes.add_students') }}</h2>
            @if($addableStudents->isEmpty())
                <p class="text-sm text-gray-400">—</p>
            @else
                <form method="POST" action="{{ route('teacher.classes.students.add', $class) }}" class="flex gap-2">
                    @csrf
                    ...
                    <button class="px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white text-sm font-semibold rounded-lg transition">{{ __('teacher.classes.add') }}</button>
                </form>
            @endif
        </div>
        @endunless
        ...
        <div class="card p-5">
            <h2 class="font-bold text-gray-900 mb-3">{{ __('teacher.classes.edit') }}</h2>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <select name="level" class="w-full rounded-lg border-gray-300 text-sm">
                        ...
                        @foreach(['beginner', 'intermediate', 'advanced'] as $level)
                            <option value="{{ $level }}" @selected($class->level === $level)>{{ __('teacher.assignments.difficulty_'.$level) }}</option>
                        @endforeach
                    </select>
                    <input type="text" name="instrument_focus" maxlength="100" class="w-full rounded-lg border-gray-300 text-sm" value="{{ old('instrument_focus', $class->instrument_focus) }}" placeholder="{{ __('teacher.classes.instrument_focus') }}">
                </div>
