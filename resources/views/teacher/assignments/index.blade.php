...
@section('title', crm_trans('assignments.title'))
...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div>
        <h1 class="text-2xl font-bold text-gray-900">{{ crm_trans($a.'.title') }}</h1>
        <p class="text-gray-500 text-sm mt-1">{{ crm_trans($a.'.subtitle') }}</p>
    </div>
    <a href="{{ crm_route('assignments.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-primary-600 hover:bg-primary-700 text-white font-semibold rounded-lg transition">
        <i data-lucide="plus" class="w-4 h-4"></i> {{ crm_trans($a.'.create') }}
    </a>
</div>

{{-- Free-tier active-assignment quota --}}
@php
    $crmQuota = app(\App\Services\Teacher\CrmQuotaService::class);
    $assignmentLimit = $crmQuota->limit(auth()->user(), 'max_active_assignments');
@endphp
@if($assignmentLimit !== -1)
    <div class="mb-6 flex flex-wrap items-center gap-3">
        <span class="px-3 py-1 rounded-full bg-purple-50 border border-purple-200 text-purple-700 text-xs font-bold">
            {{ __('teacher.limits.assignments_counter', ['used' => $crmQuota->activeAssignmentCount(auth()->user()), 'limit' => $assignmentLimit]) }}
        </span>
        <span class="text-xs text-gray-500">{{ __('teacher.limits.assignment_ai_premium') }}</span>
    </div>
@endif
@if($errors->has('limit'))
    <div class="mb-6 px-4 py-3 bg-orange-50 border border-orange-200 rounded-xl text-sm text-orange-700">
        {{ $errors->first('limit') }}
    </div>
@endif
...
<div class="card overflow-hidden">
    @if($assignments->isEmpty())
        <div class="p-10 text-center text-gray-500">
            ...
            <p class="text-sm">{{ crm_trans($a.'.no_assignments') }}</p>
        </div>
    @else
        <div class="overflow-x-auto"><table class="w-full">
            <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                    <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase">{{ crm_trans($a.'.field_title') }}</th>
                ...
                @foreach($assignments as $assignment)
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-5 py-3.5">
                            <a href="{{ crm_route('assignments.show', $assignment) }}" class="text-sm font-semibold text-gray-900 hover:text-primary-700">
                                {{ $assignment->title }}
                            </a>
                            <p class="text-xs text-gray-400">{{ $assignment->practice_type ?? crm_trans($a.'.type_practice_goal') }} · {{ $assignment->question_count }} Q</p>
                            @if($assignment->practice_type && in_array($assignment->status, ['draft', 'completed', 'sent']))
                                <a href="{{ crm_route('assignments.preview', $assignment) }}"
                                   class="inline-flex items-center gap-1 mt-1 text-[11px] font-semibold text-primary-600 hover:text-primary-800">
                                    <i data-lucide="eye" class="w-3 h-3"></i> {{ crm_trans($a.'.preview') }}
                                </a>
                            @endif
                        </td>
                        <td class="px-5 py-3.5 text-xs text-gray-500 hidden sm:table-cell">{{ crm_trans($a.'.type_'.$assignment->type) }}</td>
