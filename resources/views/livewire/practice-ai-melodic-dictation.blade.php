<div>
    ...
    <style>
        .md-dur-btn {
            ...
            background:#4f46e5; color:white; border:2px solid transparent;
            display:flex; align-items:center; justify-content:center;
            cursor:pointer; transition:background 0.12s, border-color 0.12s, box-shadow 0.12s;
        }
        .md-dur-btn:hover { background:#4338ca; }
        .md-dur-btn:active { background:#312e81; }
        .md-dur-btn.md-selected {
            background:#1e1b4b; border-color:#818cf8;
        ...
        .md-note-btn:hover { border-color:#818cf8; background:#eef2ff; }
        .md-note-btn.md-flash { background:#4f46e5; color:white; border-color:#4f46e5; transform:scale(0.93); }
        .md-oct-btn {
        ...
        .md-acc-btn.md-acc-active { background:#4f46e5; border-color:#818cf8; box-shadow:0 0 0 2px rgba(129,140,248,0.35); }
        .md-act-btn {
        ...
        .md-seg-dot.active { background:#6366f1; }
    ...
    <main class="max-w-4xl mx-auto px-1 sm:px-6 lg:px-8 pt-3.5 pb-6">
        ...
        @if(!$currentPractice)
            <div class="card p-12 text-center">
                ...
                <p class="text-gray-500 mb-4">Your AI session has expired or no melodic dictation questions were generated.</p>
                <a href="/ai-exercises" class="btn-primary text-white font-semibold py-2.5 px-6 rounded-lg inline-flex items-center gap-2">
                    <i data-lucide="sparkles" class="w-4 h-4"></i> Back to AI Exercises
                </a>
            </div>
        @else
        ...
        <div class="card overflow-hidden mb-6">
            ...
            <div class="hero-gradient py-5 px-6">
                <div class="relative flex items-center justify-center">
                    <a href="/ai-exercises" class="absolute left-0 top-1/2 -translate-y-1/2 w-12 h-12 flex items-center justify-center rounded-xl bg-white/10 hover:bg-white/20 text-white transition-all">
                        <i data-lucide="arrow-left" class="w-6 h-6"></i>
                    </a>
                    <div class="text-center">
                        <h1 class="text-xl font-bold text-white inline-flex items-center gap-2">
            ...
            <div class="p-2.5 sm:p-6">
                ...
                <div class="mb-5">
                    ...
                    <div id="correctMelodyReveal" class="hidden mt-3">
                        <p class="text-xs font-semibold text-green-700 text-center mb-1">Correct melody</p>
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="min-w-0" style="width:42%">
                            ...
                            <p class="text-sm font-bold text-gray-800 mb-2" id="beatDisplay">
                                Bar 1 / {{ explode('/', $practiceTimeSig)[0] }} beats
                            </p>
                            <div class="bg-gray-200 rounded-full overflow-hidden" style="height:6px;width:50%">
                                <div id="beatProgress" class="bg-indigo-400 h-full rounded-full transition-all duration-150" style="width:0%"></div>
                            </div>
                        </div>
                        ...
                        <div class="flex-shrink-0" style="min-width:150px;">
                            <button id="playButton"
                                style="background:linear-gradient(135deg,#4f46e5 0%,#7c3aed 100%);transition:all 0.2s;width:100%;"
                            ...
                            @if ($currentPracticeIndex < (count($practices) - 1))
                                <button id="nextPracticeBtn" x-on:click="$wire.getNextPractice()"
                                    class="font-semibold py-3 px-6 rounded-xl items-center justify-center gap-2 bg-indigo-100 text-indigo-700 border-2 border-indigo-300 hover:bg-indigo-200"
                                    style="display:none;width:100%;">
                                    <i data-lucide="arrow-right" class="w-5 h-5"></i> Next
                                </button>
                            @else
                                <a href="/ai-exercises" id="nextPracticeBtn"
                                    class="font-semibold py-3 px-6 rounded-xl items-center justify-center gap-2 bg-indigo-100 text-indigo-700 border-2 border-indigo-300 hover:bg-indigo-200"
                                    style="display:none;width:100%;">
                ...
                <div class="flex flex-wrap gap-2 mb-1 md-action-bar">
                    <button id="playMyAnswer" class="md-act-btn">
                        <i data-lucide="volume-2" class="w-4 h-4"></i> Play my melody
                    </button>
                    ...
                    <button id="nextSegBtn" class="md-act-btn" style="display:none;background:linear-gradient(135deg,#4f46e5,#6366f1);color:white;border-color:transparent;">
                        <i data-lucide="arrow-right" class="w-4 h-4"></i> Next Segment
                    </button>
                    <!-- Play from start (shown after all segments answered) -->
                    <button id="playFullPiece" class="md-act-btn" style="display:none;background:linear-gradient(135deg,#4338ca,#818cf8);color:white;border-color:transparent;">
                        <i data-lucide="rewind" class="w-4 h-4"></i> Play from start
                    </button>
                </div>
        ...
        window.initPracticeAiMelodicDictation = function() {
            ...
            function updateProgress() {
                ...
                if (beatDisplay) beatDisplay.textContent = `Bar ${curBar} · ${beatsRound} / ${tNum} beats`;
            ...
            async function playCurrentSegment() {
                ...
                playButton.innerHTML = '<i data-lucide="loader" class="w-5 h-5 inline animate-spin"></i> Loading...';
                if (typeof lucide !== 'undefined') lucide.createIcons();
                ...
                playButton.innerHTML = '<i data-lucide="loader" class="w-5 h-5 inline animate-spin"></i> Count-in...';
                if (typeof lucide !== 'undefined') lucide.createIcons();
        ...
        }; // end initPracticeAiMelodicDictation

        document.addEventListener('livewire:init', function() {
            window.initPracticeAiMelodicDictation();
            Livewire.on('practice-updated', () => setTimeout(() => window.initPracticeAiMelodicDictation(), 50));
        });
        document.addEventListener('DOMContentLoaded', function() {
            if (typeof Livewire === 'undefined') window.initPracticeAiMelodicDictation();
        });
        </script>
