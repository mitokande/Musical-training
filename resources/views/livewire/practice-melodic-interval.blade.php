    <!-- Main Content -->
    <main wire:id="practice-melodic-interval-{{ $currentPracticeIndex }}" class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <!-- Practice Card -->
        ...
        <script>
            ...
            function vfStemDir(noteKey, clef) {
                ...
                // Treble middle line B4=71; Bass middle line D3=50
                const mid = clef === 'bass' ? 50 : 71;
                return midi >= mid ? -1 : 1;
            }

            function buildStaveNote(key, dur, clef) {
                const { StaveNote, Stem } = Vex.Flow;
                const sd = vfStemDir(key, clef);
                return new StaveNote({ keys: [key], duration: dur, stem_direction: sd });
            }

            // Draw only note1 initially; called again after answer to show both.
            function drawMelodicStave(note1Key, note2Key, showBoth, clef) {
                ...
                const dur  = showBoth ? 'h' : 'w';
                const notes = keys.map(k => buildStaveNote(k, dur, clef));

                const numBeats = showBoth ? 2 : 4;
                const beatVal  = showBoth ? 2 : 4;
            ...
            window.initPracticeMelodicInterval = function() {
                ...
                if (typeof Vex !== 'undefined' && note1Key) {
                    drawMelodicStave(note1Key, note2Key, false, clef);
                }
                        ...
                        btn.onclick = async function() {
                            ...
                            try {
                                ...
                                if (typeof Vex !== 'undefined' && note1Key && note2Key) {
                                    drawMelodicStave(note1Key, note2Key, true, clef);
                                }
            ...
            document.addEventListener('livewire:init', function() {
                window.initPracticeMelodicInterval();
                Livewire.on('practice-updated', () => {
                    setTimeout(() => window.initPracticeMelodicInterval(), 50);
                });
            });
