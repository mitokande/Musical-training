    <!-- Main Content -->
    <main wire:id="practice-interval-construction-{{ $currentPracticeIndex }}" class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <!-- Practice Card -->
        <div class="card overflow-hidden mb-6">
            ...
            <div class="hero-gradient p-6">
                <div class="relative flex items-center justify-center">
                    ...
                    <div class="flex items-center gap-4 text-center">
                        <div>
                            <h1 class="text-xl font-bold text-white">Interval Construction Practice</h1>
                            <p class="text-white/80 text-sm">Build the interval by selecting the second note</p>
                        </div>
                    </div>
            ...
            <div class="p-8">
                ...
                <div class="text-center mb-6">
                    <p class="text-gray-500 text-sm mb-1">Build this interval from {{ strtoupper($currentPractice->note1) }}:</p>
                    <h2 class="text-3xl font-bold text-purple-600">{{ $currentPractice->interval }}</h2>
                </div>

                <!-- VexFlow Note Display - Shows only note1 initially -->
                <div id="noteDisplayContainer" class="w-full h-32 bg-gray-50 border-2 border-dashed border-gray-200 rounded-xl flex items-center justify-center mb-8">
                    <div class="flex flex-col items-center">
                        <div id="output" style="width: 100%; height: 180px; display: flex; justify-content: center;" 
                             ...
                             data-note2="{{ strtolower($currentPractice->note2) . '/' . $currentPractice->octave }}">
                        </div>
                    </div>
                </div>
                ...
                <div class="card p-6 mb-8">
                    <div class="flex flex-col items-center">
                        <div class="flex">
                            ...
                            data-note2="{{ strtoupper($currentPractice->note2) . $currentPractice->octave }}"
                        >
                        ...
                        <p id="playStatus" class="text-sm text-gray-500">Listen to the starting note, then select the note to complete the interval</p>
                    </div>
                </div>

                <!-- Answer Options - Note Names -->
                <div id="answerOptions" class="grid grid-cols-4 sm:grid-cols-6 gap-3" 
                     data-target="{{ $currentPractice->note2 }}"
                     data-practice-id="{{ $currentPractice->id }}"
                     data-octave="{{ $currentPractice->octave }}">
                    <button class="answer-btn card p-4 text-center font-bold text-gray-700 hover:shadow-md transition-all text-lg" data-answer="C">C</button>
                    <button class="answer-btn card p-4 text-center font-bold text-gray-700 hover:shadow-md transition-all text-lg" data-answer="C#">C#</button>
                    <button class="answer-btn card p-4 text-center font-bold text-gray-700 hover:shadow-md transition-all text-lg" data-answer="D">D</button>
                    <button class="answer-btn card p-4 text-center font-bold text-gray-700 hover:shadow-md transition-all text-lg" data-answer="D#">D#</button>
                    <button class="answer-btn card p-4 text-center font-bold text-gray-700 hover:shadow-md transition-all text-lg" data-answer="E">E</button>
                    <button class="answer-btn card p-4 text-center font-bold text-gray-700 hover:shadow-md transition-all text-lg" data-answer="F">F</button>
                    <button class="answer-btn card p-4 text-center font-bold text-gray-700 hover:shadow-md transition-all text-lg" data-answer="F#">F#</button>
                    <button class="answer-btn card p-4 text-center font-bold text-gray-700 hover:shadow-md transition-all text-lg" data-answer="G">G</button>
                    <button class="answer-btn card p-4 text-center font-bold text-gray-700 hover:shadow-md transition-all text-lg" data-answer="G#">G#</button>
                    <button class="answer-btn card p-4 text-center font-bold text-gray-700 hover:shadow-md transition-all text-lg" data-answer="A">A</button>
                    <button class="answer-btn card p-4 text-center font-bold text-gray-700 hover:shadow-md transition-all text-lg" data-answer="A#">A#</button>
                    <button class="answer-btn card p-4 text-center font-bold text-gray-700 hover:shadow-md transition-all text-lg" data-answer="B">B</button>
                </div>
        ...
        <script>
            // Store VexFlow context globally for this practice
            let vexContext = null;
            let vexStave = null;
            ...
            window.initPracticeIntervalConstruction = function() {
                ...
                // Initialize VexFlow - show only note1 initially
                if (typeof Vex !== 'undefined') {
                    ...
                    if (div) {
                        ...
                        vexStave = new Stave(10, 30, 280);
                        vexStave.addClef("treble");
                        vexStave.setContext(vexContext).draw();
                        
                        const note1 = div.dataset.note1;
                        if (note1) {
                            // Show only the first note initially (use whole note for single note)
                            const notes = [new StaveNote({ keys: [note1], duration: "w" })];
                ...
                // Function to add the second note to the display
                window.showSecondNoteOnStave = function(note2Key) {
                    if (showSecondNote) return; // Already shown
                    showSecondNote = true;
                    
                    const div = document.getElementById("output");
                    if (div && typeof Vex !== 'undefined') {
                        const { Renderer, Stave, StaveNote, Voice, Formatter } = Vex.Flow;
                        
                        // Re-render with both notes
                        div.innerHTML = '';
                        const renderer = new Renderer(div, Renderer.Backends.SVG);
                        renderer.resize(300, 180);
                        ...
                        const note1 = div.dataset.note1;
                        const notes = [
                            new StaveNote({ keys: [note1], duration: "h" }),
                            new StaveNote({ keys: [note2Key], duration: "h" })
                        ];
                ...
                if (playButton && answerOptions) {
                    ...
                    // Play button click handler - plays only the first note
                    playButton.onclick = function() {
                        const note1 = this.dataset.note1;
                        const audioUrl = `https://mithatck.com/music/api/note.php?note=${note1}&duration=2`;
                    ...
                    answerButtons.forEach(btn => {
                        btn.onclick = async function() {
                            ...
                            // Play the selected note and show it on the stave
                            const selectedNoteAudio = `https://mithatck.com/music/api/note.php?note=${answer}${octave}&duration=2`;
                            const audio = new Audio(selectedNoteAudio);
                            audio.play();
                            
                            // Show the selected note on the stave
                            window.showSecondNoteOnStave(answer.toLowerCase() + '/' + octave);
                            ...
                            try {
                                const response = await fetch('/api/practice/check-answer', {
                                    ...
                                    body: JSON.stringify({
                                        practice_id: 6, // Interval Construction Practice ID
                                        answer: answer,
                                        target: target
                                    })
                                });
