...
@section('content')
<div x-data="{ contentType: '{{ old('content_type', $article->content_type) }}', seoOpen: false }" class="max-w-4xl mx-auto space-y-6">
    {{-- Header --}}
    ...
    <form action="{{ route('admin.content.update', $article) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="slug" id="slug" x-ref="slugField" value="{{ old('slug', $article->slug) }}" required
                ...
                <div>
                    ...
                    <select name="visibility" id="visibility" class="w-full rounded-lg border-gray-300 focus:border-purple-500 focus:ring-purple-500">
                        ...
                        <option value="premium" {{ old('visibility', $article->visibility) === 'premium' ? 'selected' : '' }}>Premium Only</option>
                        <option value="private" {{ old('visibility', $article->visibility) === 'private' ? 'selected' : '' }}>Private</option>
                    </select>
            ...
            <div>
                ...
                <textarea name="body" id="body" rows="12"
                    class="w-full rounded-lg border-gray-300 focus:border-purple-500 focus:ring-purple-500">{{ old('body', $article->body) }}</textarea>
            ...
            <div>
                ...
                <textarea name="excerpt" id="excerpt" rows="3"
                    class="w-full rounded-lg border-gray-300 focus:border-purple-500 focus:ring-purple-500">{{ old('excerpt', $article->excerpt) }}</textarea>
            ...
            <div>
                ...
                @if($article->featured_image)
                    <div class="mb-2">
                        <img src="{{ asset('storage/' . $article->featured_image) }}" alt="Current image" class="h-24 w-auto rounded-lg object-cover">
                    </div>
                @endif
                ...
                <div class="pt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="meta_description" id="meta_description" rows="2"
                            class="w-full rounded-lg border-gray-300 focus:border-purple-500 focus:ring-purple-500">{{ old('meta_description', $article->meta_description) }}</textarea>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="url" name="canonical_url" id="canonical_url" value="{{ old('canonical_url', $article->canonical_url) }}"
                            class="w-full rounded-lg border-gray-300 focus:border-purple-500 focus:ring-purple-500">
                    </div>
                    <div class="md:col-span-2">
                        <label for="og_image" class="block text-sm font-medium text-gray-700 mb-1">OG Image URL</label>
                        <input type="url" name="og_image" id="og_image" value="{{ old('og_image', $article->og_image) }}"
                            class="w-full rounded-lg border-gray-300 focus:border-purple-500 focus:ring-purple-500">
                    </div>
                </div>
            </div>
        ...
        <div class="flex items-center justify-end gap-3">
            ...
            <button type="submit" class="inline-flex items-center gap-2 px-6 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition">
                <i data-lucide="save" class="w-4 h-4"></i> Update Content
            </button>
        </div>
    </form>
</div>
