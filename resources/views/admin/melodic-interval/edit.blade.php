...
<div class="max-w-2xl" x-data="intervalEditor('{{ $practice->note1 }}', {{ (int)$practice->octave }}, '{{ $practice->note2 }}', {{ (int)($practice->note2_octave ?? $practice->octave) }})">
    ...
    <div class="mb-8">
        ...
        <div class="flex items-center gap-3 mb-2">
            ...
            <h1 class="text-2xl font-bold text-gray-900">Edit Melodic Interval Practice</h1>
        </div>
        <p class="text-gray-500">Update melodic interval recognition question #{{ $practice->id }}</p>
    </div>

    <!-- Form -->
    <form action="{{ route('admin.melodic-interval.update', $practice) }}" method="POST" class="card p-6 space-y-6">
        @csrf
        @method('PUT')
    ...
    <div class="mt-4 p-4 bg-red-50 border border-red-200 rounded-xl">
        <div class="flex items-center justify-between">
            ...
            <form action="{{ route('admin.melodic-interval.destroy', $practice) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this practice?')">
                @csrf
                @method('DELETE')
