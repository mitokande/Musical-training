...
<div class="max-w-2xl space-y-6">
    ...
    <a href="{{ route('admin.appointments.show', $appointment) }}" class="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-purple-600 transition-colors">
        <i data-lucide="arrow-left" class="w-4 h-4"></i> Back to Appointment
    </a>

    <div class="card p-6">
        <div class="flex items-center gap-2 mb-6">
            <i data-lucide="pencil" class="w-5 h-5 text-purple-600"></i>
            <h2 class="text-lg font-semibold text-gray-900">Edit Appointment</h2>
        </div>
        ...
        <form action="{{ route('admin.appointments.update', $appointment) }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3"
                          class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500">{{ old('description', $appointment->description) }}</textarea>
                ...
                <div>
                    ...
                    <select name="student_id" id="student_id" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500">
                        ...
                        @foreach($students ?? [] as $student)
                            <option value="{{ $student->id }}" {{ old('student_id', $appointment->student_id) == $student->id ? 'selected' : '' }}>{{ $student->name }}</option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <select name="school_id" id="school_id" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500">
                    ...
                    @foreach($schools ?? [] as $school)
                        <option value="{{ $school->id }}" {{ old('school_id', $appointment->school_id) == $school->id ? 'selected' : '' }}>{{ $school->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="datetime-local" name="starts_at" id="starts_at"
                           value="{{ old('starts_at', $appointment->starts_at ? $appointment->starts_at->format('Y-m-d\TH:i') : '') }}" required
                           class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500">
                ...
                <div>
                    ...
                    <input type="datetime-local" name="ends_at" id="ends_at"
                           value="{{ old('ends_at', $appointment->ends_at ? $appointment->ends_at->format('Y-m-d\TH:i') : '') }}" required
                           class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="type" id="type" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500">
                        <option value="lesson" {{ old('type', $appointment->type) == 'lesson' ? 'selected' : '' }}>Lesson</option>
                        <option value="consultation" {{ old('type', $appointment->type) == 'consultation' ? 'selected' : '' }}>Consultation</option>
                        <option value="trial" {{ old('type', $appointment->type) == 'trial' ? 'selected' : '' }}>Trial</option>
                        <option value="meeting" {{ old('type', $appointment->type) == 'meeting' ? 'selected' : '' }}>Meeting</option>
                    </select>
            ...
            <div>
                ...
                <input type="url" name="meeting_url" id="meeting_url" value="{{ old('meeting_url', $appointment->meeting_url) }}"
                       class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500">
            ...
            <div>
                ...
                <textarea name="notes" id="notes" rows="3"
                          class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500">{{ old('notes', $appointment->notes) }}</textarea>
            ...
            <div class="pt-4 border-t border-gray-200 flex items-center gap-3">
                <button type="submit" class="btn-primary px-6 py-2.5 text-white text-sm font-semibold rounded-lg transition-all hover:shadow-lg">
                    <i data-lucide="save" class="w-4 h-4 inline mr-1"></i> Update Appointment
                </button>
                <a href="{{ route('admin.appointments.show', $appointment) }}" class="px-6 py-2.5 text-sm font-medium text-gray-600 hover:text-gray-800">Cancel</a>
            </div>
        </form>
    </div>
</div>
