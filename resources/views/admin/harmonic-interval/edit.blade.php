...
<div class="max-w-2xl" x-data="intervalEditor('{{ $practice->note1 }}', {{ (int)$practice->octave }}, '{{ $practice->note2 }}', {{ (int)($practice->note2_octave ?? $practice->octave) }})">
    ...
    <div class="mb-8">
        ...
        <div class="flex items-center gap-3 mb-2">
            ...
            <h1 class="text-2xl font-bold text-gray-900">Edit Harmonic Interval Practice</h1>
        </div>
        <p class="text-gray-500">Update harmonic interval recognition question #{{ $practice->id }}</p>
    </div>

    <!-- Form -->
    <form action="{{ route('admin.harmonic-interval.update', $practice) }}" method="POST" class="card p-6 space-y-6">
        @csrf
        @method('PUT')

        <div>
            <label for="interval" class="block text-sm font-semibold text-gray-700 mb-2">Interval Type *</label>
            <select name="interval" id="interval" class="w-full px-4 py-3 border border-gray-300 rounded-xl text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" required>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <select name="note1" id="note1" x-model="note1" class="w-full px-4 py-3 border border-gray-300 rounded-xl text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" required>
            ...
            <div>
                ...
                <select name="note2" id="note2" x-model="note2" class="w-full px-4 py-3 border border-gray-300 rounded-xl text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" required>
                    @foreach (['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B'] as $note)
        ...
        <div>
            ...
            <select name="octave" id="octave" x-model.number="octave1" class="w-full px-4 py-3 border border-gray-300 rounded-xl text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" required>
    ...
    <div class="mt-4 p-4 bg-red-50 border border-red-200 rounded-xl">
        <div class="flex items-center justify-between">
            ...
            <form action="{{ route('admin.harmonic-interval.destroy', $practice) }}" method="POST" onsubmit="return confirm('Are you sure?')">
                @csrf
                @method('DELETE')
