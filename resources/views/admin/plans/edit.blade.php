...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <form action="{{ route('admin.plans.update', $plan) }}" method="POST" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="slug" id="slug" x-ref="slugField" value="{{ old('slug', $plan->slug) }}" required
            ...
            <div>
                ...
                <select name="role" id="role" required class="w-full rounded-lg border-gray-300 focus:border-purple-500 focus:ring-purple-500">
                    ...
                    <option value="user" {{ old('role', $plan->role) === 'user' ? 'selected' : '' }}>User</option>
                    <option value="teacher" {{ old('role', $plan->role) === 'teacher' ? 'selected' : '' }}>Teacher</option>
                    <option value="school" {{ old('role', $plan->role) === 'school' ? 'selected' : '' }}>School</option>
                </select>
            ...
            <div>
                ...
                <select name="currency" id="currency" class="w-full rounded-lg border-gray-300 focus:border-purple-500 focus:ring-purple-500">
                    ...
                    <option value="TRY" {{ old('currency', $plan->currency) === 'TRY' ? 'selected' : '' }}>TRY</option>
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="3"
                class="w-full rounded-lg border-gray-300 focus:border-purple-500 focus:ring-purple-500">{{ old('description', $plan->description) }}</textarea>
        ...
        <div>
            ...
            <textarea name="features" id="features" rows="4" placeholder='["Feature 1", "Feature 2", "Feature 3"]'
                class="w-full rounded-lg border-gray-300 focus:border-purple-500 focus:ring-purple-500 font-mono text-sm">{{ old('features', is_array($plan->features) ? json_encode($plan->features, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) : $plan->features) }}</textarea>
        ...
        <div class="flex items-center justify-end gap-3 pt-4 border-t">
            ...
            <button type="submit" class="inline-flex items-center gap-2 px-6 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition">
                <i data-lucide="save" class="w-4 h-4"></i> Update Plan
            </button>
        </div>
    </form>
</div>
