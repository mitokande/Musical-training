...
class PracticeHarmonicInterval extends Component
{
    ...
    public function mount($practices)
    {
        ...
        if (! empty($settings)) {
            ...
            // Octave range is derived from the selected clef (CLEF_RANGES);
            // notes never leave the clef's playable range.
    ...
    {
        $practiceId = Practice::where('slug', 'harmonic-interval-practice')->value('id');
